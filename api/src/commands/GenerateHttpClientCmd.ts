...
const _newAxiosImport = `import axios, {
	AxiosInstance,
	AxiosRequestConfig,
	HeadersDefaults,
	ResponseType,
...
export class GenerateHttpClientCmd implements CommandProvider {
	...
	private async loadPlatformModule(): Promise<any> {
		let platform = await importPackage(
		...
		throw new Error(
			"Unsupported platform. Please use Express.js or Koa.js platform."
		);
	}
	...
	private async generate($ctx: GenerateHttpClientCtx) {
		...
		const platform: { injector: InjectorService; stop: () => Promise<any> } =
	...
	private async generateFromSpec(
		spec: any,
		conf: any,
		$ctx: GenerateHttpClientCtx
	) {
		const operationIdMode =
			!conf.operationId || conf.operationId === "%c_%m"
				? "underscore"
				: "default";
		...
		const promises = files.map(({ content, name }) => {
			...
			console.log(
				`${$ctx.output}/${name}`,
				path.resolve(`${$ctx.output}/${name}`)
			);
			return this.fs.writeFile(`${$ctx.output}/${name}`, content, {
				encoding: "utf8",
			});
		});
	...
	private onParseSchema(originalSchema: any, parsedSchema: any) {
		...
		if (isString(content)) {
			...
			parsedSchema.content = parsedSchema.content.replace(
				"object",
				"Record<string, any>"
			);
		}
	...
	private createRouteName(
		routeNameInfo: RouteNameInfo,
		raw: RawRouteInfo,
		mode: "default" | "underscore" = "default"
	) {
		let operationId = "";
		...
		const name = camelCase(
			this.options.transformOperationId
				? this.options.transformOperationId(operationId, routeNameInfo, raw)
				: operationId
		);
